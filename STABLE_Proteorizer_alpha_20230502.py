...
###########################################################     #    #  #  #   # #  ## #  # #      #	###########################################################
###########################################################     #### #  # #    # #   # #### #### ####	###########################################################
# 1) Uses Postgres (psql) to handle uniprot data which should increase the speed of STEP 3 and 4 immensely
...
def MECHISMO_GETTER(uniprot_identifier):
	...
	for getter in filepaths:
		if uniprot_identifier in getter:
			mechismofile = getter.replace("\n","")
	...
	#print pp(mechismo)
	return mechismo_dict, filepaths
...
def fetch_BROT(inputa):
	...
	return lengelenge
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
mechismo, fpaths = MECHISMO_GETTER(test_unip)
...
# alignments={}
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def HOMOL_GETTER(uniprot_identifier):
...
if erledigt_two == "FALSE":
	alignments = HOMOL_GETTER(test_unip)
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def HOMOL_UNIP_GETTER(inputdictionary_one, inputdictionary_two, inputdictionary_three, uniprot_identifier):
		...
		if protein == uniprot_identifier:
			...
			for result in cursor.fetchall():
				...
				if inputdictionary_two["UniProt"].has_key(protein)==False:
					...
					inputdictionary_two["UniProt"][protein][homol_mutat]=kommentar		
	return inputdictionary_two, inputdictionary_three
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
homol_counts, count_dict = HOMOL_UNIP_GETTER(alignments, homol_counts,count_dict, test_unip)
										...
										if "GLY" in residue1:
											...
											if "GLY" in residue2:		### to look at
												...
												if "AF-" in structure:
													...
													if float(bval1) >= 70.0:		### "Confident" Alpha Zero predictions start at 70/100
														if float(bval2) >= 70.0: 	### I only check the CA of the main chain for confidence
												...
												if "AF-" in structure:
													...
													if float(bval1) >= 70.0:	### "Confident" Alpha Zero predictions start at 70/100
														if float(bval3) >= 70.0: ### I only check the CA of the main chain for confidence
											...
											if "GLY" in residue2:
												...
												if "AF-" in structure:
													bval3 = atom3.get_bfactor()
													bval2 = atom2.get_bfactor()
													if float(bval3) >= 70.0:	### "Confident" Alpha Zero predictions start at 70/100
														if float(bval2) >= 70.0: ### I only check the CA of the main chain for confidence
															distances[index][b].append(min(check_dist))
												else:
													distances[index][b].append(min(check_dist))
					...
					if totalcount not in forbidden:
						if poi in namus:
						    ...
						    if float(Konserve[startnumberlabel][0])>= 0.7:
						    	dwg.add(dwg.rect((x,y),(10,len(sequences_dict)*20), fill= clustaltypes[Clustalcolors[letter.upper()]], opacity=0.2))
...
#   def create_svg(sequences_dict, projectname, colordict, startposition, windowsize, poi, goi):
	if "," in test_mutat_raw:
		create_svg(alignments, project, colordict, test_mutat_raw, 30, test_unip, test_gn, TheForbiddenPositions)	### works with Single Mutation as input only, so far
		create_svg(alignments, project, colordict, test_mutat_raw, 50, test_unip, test_gn, TheForbiddenPositions)	### works with Single Mutation as input only, so far
		create_svg(alignments, project, colordict, test_mutat_raw, 30000, test_unip, test_gn, TheForbiddenPositions)	### works with Single Mutation as input only, so far
	else:
		input_position = nums.search(str(test_mutat)).group(0)
		create_svg(alignments, project, colordict, input_position, 30, test_unip, test_gn, TheForbiddenPositions)	### works with Single Mutation as input only, so far
		create_svg(alignments, project, colordict, input_position, 50, test_unip, test_gn, TheForbiddenPositions)	### works with Single Mutation as input only, so far
		create_svg(alignments, project, colordict, input_position, 30000, test_unip, test_gn, TheForbiddenPositions)	### works with Single Mutation as input only, so far
except:
	logging.exception("message")
	pass
...
### Here we introduce bayesian scoring
def bayesian_scoring(seqval, typval, protclustaval, clustmembval,evidtotalval,evidclustaval):
        seq_score       = 0
        typus_score     = 0
        protclusta_score= 0
        clustmemb_score = 0
...
### bayes score is sum of log2(TPR/FPR)
	if "RW" in clustermethod:
		metricfilepath = "/net/home.isilon/ag-russell/bq_tschmenger/PhD/MechismoScanner/PERTURBED_INTERFACES/EnrichmentProbability/Hereditary_Cancer/3D_Clustering_For_Any_Variant/CONNECTOR_Scoring/Version3/RandomWalk_Metrics_20230323.txt"
	elif "HClust" in clustermethod:
		metricfilepath = "/net/home.isilon/ag-russell/bq_tschmenger/PhD/MechismoScanner/PERTURBED_INTERFACES/EnrichmentProbability/Hereditary_Cancer/3D_Clustering_For_Any_Variant/CONNECTOR_Scoring/Version_HClust/HClust_Metrics_20230323.txt"
	else:
		metricfilepath = "/net/home.isilon/ag-russell/bq_tschmenger/PhD/MechismoScanner/PERTURBED_INTERFACES/EnrichmentProbability/Hereditary_Cancer/3D_Clustering_For_Any_Variant/CONNECTOR_Scoring/Version3/RandomWalk_Metrics_20230323.txt"
	...
	collected = {"TotalEvidence":[],"Clustermembers":[],"ProteinClusters":[],"SeqIdentity":[], "TypeIdentity":[],"ClusterEvidence":[]}
	...
	try:
		...
		try:                                            
                ...
                try:
                    ...
                    pass
		bayesianscore = float(seq_score+typus_score+protclusta_score+clustmemb_score+evid_total_score+evid_clusta_score)
		...
		#	NoClusterMembership 	Input 	167 	F167Y
				if "#" not in line:
					if "Data_Source" not in line:
						...
						if "-" in type_id:
                                                        type_id = "0"
						info		= position+"-"+typus+"-"+seq_id+"-"+type_id+"-"+str(evidencecount)
						if typus == "Input":
...
try:
	...
	for vect in vectors:
		...
		seq             = vect.split("-")[6].replace(" ","").replace("\n","")
                ...
                protclusta      = vect.split("-")[9].replace(" ","").replace("\n","")
                clustmemb       = vect.split("-")[10].replace(" ","").replace("\n","")
                evid_total      = vect.split("-")[11].replace(" ","").replace("\n","")
                evid_clusta     = vect.split("-")[12].replace(" ","").replace("\n","")
		bayes		= bayesian_scoring(seq,typus,protclusta,clustmemb,evid_total,evid_clusta)
		...
		for line in datafile:
			if "Clusternumber" not in line:
				if "#" not in line:
					...
					position = line.split("\t")[2].replace(" ","")
					if "Input" in typus: 
						print line.replace("\n",""),"\t",asdrubael[str(position)]
					else:
						print line.replace("\n",""),"\t","-"
			else:
 				print "Clusternumber","\t","Data_Source","\t","Position","\t","Functional_Information","\t","Mechismo_Predictions","\t","SeqIdent%","\t","TypeIdent%","\t","BAYES"
