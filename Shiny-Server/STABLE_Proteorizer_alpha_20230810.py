...
try:
	...
	for vect in vectors:
		...
		bayes		= str(round(float(bayesian_scoring(seq,typus,protclusta,clustmemb,evid_total,evid_clusta)),2))
		if databank == "Uniprot":
			randomforestvector = [seq,typus,evidence, protclusta,clustmemb,evid_total,evid_clusta,mutscore]
			rfcmd = "/usr/bin/python3 /net/home.isilon/ag-russell/bq_tschmenger/PhD/MechismoScanner/PERTURBED_INTERFACES/EnrichmentProbability/Hereditary_Cancer/3D_Clustering_For_Any_Variant/MachineLearning_Test/ML_Proteorizer_AppPredictor_20230810.py "+str(randomforestvector).replace(" ","")+" "+machinelearning_savefile+" "+str(position)+" "+str(clustermethod)
			try:
				os.system(rfcmd)
			except:
				pass
				...
				if "#" not in line:
					...
					if "Input" in typus: 
						...
						try:
							...
							hetcnt = 0
							homcnt = 0	
						if float(asdrubael[str(position)][0])>=15:
							if float(asdrubael[str(position)][1])>=0.75:
								verdict = "Impact"
							else:
								verdict = "Uncertain"
						elif float(asdrubael[str(position)][0])<15:
							if float(asdrubael[str(position)][1])>=0.75:
								verdict = "Uncertain"
							else:
								verdict = "No_Impact"		 										
						print line.replace("\n",""),"\t",asdrubael[str(position)][0],"\t",cosmiccnt,"\t",hetcnt,"\t",homcnt,"\t",asdrubael[str(position)][1],"\t",verdict
					else:
