...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def HOMOL_UNIP_GETTER(inputdictionary_one, inputdictionary_two, inputdictionary_three, uniprot_identifier, inputdictionary_four):
	...
	for protein in inputdictionary_one:
		if protein == uniprot_identifier:
			...
			############
			try:
				commander_farsight = "SELECT variant, ac, het, hom, an FROM naturalvariants WHERE(unip=\'"+protein+"');"
				cursor.execute(commander_farsight)
				for result in cursor.fetchall():
					variant, allelecount, hetcount, homcount, allelenumber = result
					kommentar = str(allelecount)+"-"+str(hetcount)+"-"+str(homcount)+"-"+str(allelenumber)
			...
			############
			try:
				commander_farsight = "SELECT count, variant FROM cosmicinfo WHERE(unip=\'"+protein+"');"
				cursor.execute(commander_farsight)
				for result in cursor.fetchall():
					count, variant = result
					if inputdictionary_four["COSMIC"].has_key(variant)==False:
						inputdictionary_four["COSMIC"][variant]=[]
						inputdictionary_four["COSMIC"][variant]=[count]
					else:
						inputdictionary_four["COSMIC"][variant].append(count)
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
homol_counts, count_dict, extra_dictionary = HOMOL_UNIP_GETTER(alignments, homol_counts, count_dict, test_unip, extra_dictionary)
...
try:
	...
	for vect in vectors:
		...
		if databank == "Uniprot":
			randomforestvector = [seq,typus,protclusta,clustmemb,evid_total,evid_clusta]
			rfcmd = "/usr/bin/python3 /net/home.isilon/ag-russell/bq_tschmenger/PhD/MechismoScanner/PERTURBED_INTERFACES/EnrichmentProbability/Hereditary_Cancer/3D_Clustering_For_Any_Variant/MachineLearning_Test/ML_Proteorizer_AppPredictor.py "+str(randomforestvector).replace(" ","")+" "+machinelearning_savefile+" "+str(position)+" "+str(clustermethod)
			try:
				os.system(rfcmd)
			except:
				pass
...
try:
	with open(machinelearning_savefile,"r") as ml_result:
		for line in ml_result:
			...
			if "1" in classified:
				rfresult = "Impact("+score+")"
			else:
				rfresult = "No_Impact("+score+")"
			asdrubael[position].append(rfresult)
except:
			asdrubael[position].append("-")
###
if "RW" in clustermethod:
			...
			if "Clusternumber" not in line:
				if "#" not in line:
					...
					if "Input" in typus: 
						...
						try:
							cosmiccnt = extra_dictionary["COSMIC"][mutat][0].replace(" ","")
						except:
							cosmiccnt = 0
						try:
							hetcnt = extra_dictionary["Hereditary"][mutat].split("-")[1]
							homcnt = extra_dictionary["Hereditary"][mutat].split("-")[2]
						except:
							hetcnt = 0
							homcnt = 0							
						print line.replace("\n",""),"\t",asdrubael[str(position)][0],"\t",cosmiccnt,"\t",hetcnt,"\t",homcnt,"\t",asdrubael[str(position)][1]
					else:
 				...
 				print "Clusternumber","\t","Data_Source","\t","Position","\t","Functional_Information","\t","Mechismo_Predictions","\t","SeqIdent%","\t","TypeIdent%","\t","BAYES","\t","COSMIC","\t","gnomAD_Het","\t","gnomAD_Hom","\t","Predictor"
except:
	pass
